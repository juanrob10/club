...
@receiver(pre_save, sender=EnrolledPackage)
def update_package(sender, instance, **kwargs):  
        ...
        instance.consumed_time = time_sesion
        m, s = divmod(time_sesion.total_seconds(), 60)
        h, m = divmod(m, 60)
        instance.consumed_hours = '{:d}:{:02d}:{:02d} Hrs'.format(int(h),int(m),int(s))
    
        limite = instance.package_type.hours
        limite_timedelta =  timedelta(days=0,hours=limite,minutes=0,seconds=0)
        remaining_time = limite_timedelta - time_sesion
        ...
        if remaining_time >= time_0:
            ...
            m, s = divmod(remaining_time.total_seconds(), 60)
            h, m = divmod(m, 60)
            instance.remaining_hours  = '{:d}:{:02d}:{:02d} Hrs'.format(int(h),int(m),int(s))
        
        else:
            instance.remaining_time = time_0
            m, s = divmod(time_0.total_seconds(), 60)
            h, m = divmod(m, 60)
            instance.remaining_hours = '{:d}:{:02d}:{:02d} Hrs'.format(int(h),int(m),int(s))
